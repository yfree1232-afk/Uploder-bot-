...
def decrypt_appx_link(enc: str) -> str:
    try:
        enc_data = b64decode(enc.split(':')[0])
...
def resolve_appx_vercel_url(url: str) -> str:
    ...
    try:
        ...
        token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Q4BxMCC6y9f14LXvGng8omlkeA5Hc1Jzw7C7exjSJGo"
                ...
                if "drive" in url:
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            print(f"Attempt {attempt+1} failed to download PDF: {e}")
                            await asyncio.sleep(2)
                    
                    if success and os.path.isfile(pdf_path):
                        try:
                        ...
                        raise FileNotFoundError(f"Download failed: PDF file for '{namef}' was not found. Please ensure the link is active.")    
                    ...
                    if '*' in url:
                        mpd = url.split('*')[0]
                        raw_key = url.split('*')[1]
                        keys_string = raw_key if raw_key.startswith('--key') else f"--key {raw_key}"
                    else:
                        mpd = url
                        keys_string = ""
                    remaining_links = len(links) - count
    ...
    try:
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    if success and os.path.isfile(pdf_path):
                        try:
                            copy = await bot.send_document(chat_id=m.chat.id, document=pdf_path, caption=cc1)
                            count += 1
                            os.remove(pdf_path)
                        except FloodWait as e:
                            await m.reply_text(str(e))
                        ...
                        raise FileNotFoundError(f"Download failed: PDF file for '{name}' was not found. Please ensure the link is active.")
